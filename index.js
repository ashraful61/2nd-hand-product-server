    ...
    try {
        ...
        */
        ...
        app.get('/productCategories', async (req, res) => {
            const query = {}
            const result = await productCategoriesCollection.find(query).project({ name: 1 }).toArray();
            res.send(result);
        })

        app.get('/products', async (req, res) => { //verifyJWT, verifyAdmin
            const query = {};
            const products = await productsCollection.find(query).toArray();
            res.send(products);
        })
     //Get products by category id
        app.get('/products/:name', async (req, res) => { //verifyJWT, verifyAdmin
        ...
        app.delete('/products/:id', async (req, res) => { //verifyJWT, verifyAdmin, 
            const id = req.params.id;
            const filter = { _id: ObjectId(id) };
